...
export class FormsService {
  ...
  async findAll(): Promise<any[]> {
    ...
    // For performance, don't retrieve GridFS data in findAll
  ...
  async findByUserId(userId: string): Promise<any[]> {
    ...
    const forms = await this.formModel.find({ userId: userId }).populate('userId').exec();
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!fileCheck.exists) {
          ...
          reject(new NotFoundException(`GridFS file not found: ${fileCheck.error}`));
          return;
        }
        
        console.log(`GridFS file found: ${fileId}, size: ${fileCheck.length} bytes`);
        ...
        if (fileCheck.length > 25 * 1024 * 1024) { // > 25MB
          console.log(`Using optimized chunk retrieval for large file (${(fileCheck.length / 1024 / 1024).toFixed(2)}MB)`);
        ...
        downloadStream.on('data', (chunk: Buffer) => {
          ...
          if (totalBytesReceived % (5 * 1024 * 1024) === 0) { // Every 5MB
            console.log(`Downloaded ${(totalBytesReceived / 1024 / 1024).toFixed(2)}MB of ${(fileCheck.length / 1024 / 1024).toFixed(2)}MB`);
          }
        });
        
        downloadStream.on('end', () => {
          try {
            console.log(`GridFS download completed: ${totalBytesReceived} bytes received`);
            ...
            if (!jsonString.trim()) {
              console.error('GridFS file contains empty or whitespace-only content');
            ...
            console.error('Failed to parse GridFS data:', {
              ...
              firstBytes: chunks.length > 0 ? chunks[0].toString('utf8', 0, Math.min(100, chunks[0].length)) : 'No data'
            });
            reject(new Error(`Failed to parse GridFS data: ${parseError.message}`));
          }
        });
        ...
        downloadStream.on('error', (error) => {
          ...
          reject(new NotFoundException(`GridFS download failed: ${error.message}`));
        });
  ...
  private async retrieveLargeFileDirectly(objectId: ObjectId, fileInfo: any): Promise<any> {
    try {
      console.log(`Starting direct chunk retrieval for large file: ${objectId}`);
      ...
      console.log(`File has ${totalChunks} chunks, total size: ${(fileInfo.length / 1024 / 1024).toFixed(2)}MB`);
      ...
      for (let i = 0; i < totalChunks; i += batchSize) {
        ...
        console.log(`Retrieving chunks ${i} to ${endIndex - 1} of ${totalChunks}`);
      ...
      const parsedData = JSON.parse(jsonString);
      console.log(`Successfully parsed large GridFS data for file: ${objectId}`);
  ...
  async diagnoseGridFSIssues(): Promise<any> {
    try {
      ...
      const formsWithGridFS = await this.formModel.find({ 
        isLargeData: true, 
  ...
  async verifyGridFSConnectivity(): Promise<any> {
    try {
      ...
      // Try to retrieve it
      const retrievedData = await this.retrieveFromGridFS(testFileId.toString());
  ...
  async emergencyRecoverFile(fileId: string): Promise<any> {
    try {
      ...
      console.log(`File exists, attempting direct chunk recovery. Size: ${(fileCheck.length / 1024 / 1024).toFixed(2)}MB`);
      ...
      try {
        ...
        const forms = await this.formModel.find({ gridFSFileId: fileId }).exec();
        const recoveryResults = [];
        ...
        for (const form of forms) {
          try {
            await this.formModel.findByIdAndUpdate(form._id, {
              ...
              $set: { 
                isLargeData: false,
                formData: { _corrupted: true, _originalSize: fileCheck.length, _error: 'Memory limit exceeded during retrieval' },
                dataSize: 0
              }
            });
  ...
  async forceOptimizedRetrieval(fileId: string): Promise<any> {
    try {
      ...
      console.log(`Forcing direct chunk retrieval for file: ${fileId}, size: ${(fileCheck.length / 1024 / 1024).toFixed(2)}MB`);
      const data = await this.retrieveLargeFileDirectly(objectId, fileCheck);
